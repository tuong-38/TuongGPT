...
def delete_conversation_by_thread_id(thread_id: str) -> bool:
    session = SessionLocal()
    try:
        # Xóa toàn bộ tin nhắn liên quan
        session.query(ChatMessage).filter(ChatMessage.thread_id == thread_id).delete()
        # Xóa bản ghi hội thoại
        session.query(Conversation).filter(Conversation.thread_id == thread_id).delete()
        session.commit()
        return True
